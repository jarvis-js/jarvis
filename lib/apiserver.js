...
ApiServer.prototype.getModules = function getModules(request, response, next) {
	response.json(200, this.jarvis.modules);
	return next();
};

ApiServer.prototype.getActions = function getActions(request, response, next) {
	var actions = {};
	for (var moduleKey in this.jarvis.modules) {
		var module = this.jarvis.modules[moduleKey];
		for (var action in module.actions) {
			actions[action] = module.actions[action];
			actions[action].match = actions[action].match.toString();
		}
	}
